...
\cabstract{
    ...
    大数据处理、大规模智能模型训练等快速增长的计算需求，使计算集群的性能愈发捉襟见肘：一方面，高性能计算（HPC）集群正在得到更广泛的部署和应用。这类集群拥有的新型高性能网络，明显地区别于传统集群。
    以智能网卡为硬件基础，高性能计算机能够在无中央处理器（CPU）的介入下，对机器内存直接读写以传输数据。这种被称为远程直接内存访问（RDMA）的传输机制，相比以太网具有低占用、低延迟、高带宽等技术优势。
    另一方面，作为基础设施的软件框架，也在面临严峻的性能挑战：当前，大多数分布式计算框架仍然缺乏对高性能硬件的支持，特别是无法充分利用集群中的高速网络资源。
    
    近年来，出现了以强化学习、流处理为代表的新型计算任务。其任务逻辑更加灵活、复杂，不再能以数据并行、任务并行简单描述。为了支持更灵活的任务调度，计算框架需要实现高效的数据管理机制，特别是针对集群中分布的内存对象。
    分布式内存存储Plasma是新型计算框架Ray的内存管理组件。然而，Plasma尚未能支持HPC集群的高速网络，从而在任务间快速地传递内存对象。
    为此，通过设计支持RDMA的高性能通信机制，我们能将现代网络的硬件优势转化为Ray系统整体的性能优势。
    
    在本研究中，我们首先测试和分析了其基于以太网和套接字传输数据而导致的性能瓶颈。然后，在高性能集群上，我们提出了一种支持RDMA特性的高吞吐对象传输机制。
    该机制针对大型数据对象，提出了基于单边读语义的传输协议，实现了用户态的内存零拷贝。并且，该机制能够在运行时，根据数据大小动态地选择传输协议，以获得最佳性能。进一步，我们基于并行计算范式MPI构造了分布式、可扩展的多节点传输性能测试。
...
\eabstract{
    ...
    the physical resources of the clusters. The growing need forg computation that comes with the mentioned workloads challenges the performance of these machines. On the one hand, 
    high-performance computing (HPC) clusters are widely deployed to handle intensified computation. These systems have distinct network architecture.
